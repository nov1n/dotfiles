...
Config.insert_section = function(symbol, total_width)
  ...
  -- Handle empty or missing commentstring
  if comment_string == "" then
    comment_string = "%s"
  end

  local content_width = total_width - (comment_string:len() - 2)

  -- Guard against negative or too small content width
  if content_width < 1 then
    vim.notify("Total width too small for comment string", vim.log.levels.WARN)
    return
  end

  local content = string.rep(symbol, content_width)
  ...
  if inner_start then
    ...
    -- Fallback: just move to the new line
